	...
	const config = {
	  info: {
		...
		authors: [
		  ...
		  {
			name: "LetsGamingDE",
			discord_id: "272402865874534400",
			github_username: "LetsGaming",
		  },
		],
		version: "2.5.0",
	  ...
	  changelog: [
		{
		  title: "Fixed",
		  type: "fixed",
		  items: [`Fixed the plugin to work with new DOM Structure of Discord.`],
		},
	  ],
	};
		  ...
		  const plugin = (Plugin, Library) => {
			...
			class Tooltip {
			  containerClassName = Utilities.className(
				"FileViewerTooltip",
				...["tooltip", "tooltipTop", "tooltipPrimary"].map(
				  (c) => DiscordClasses.Tooltips?.[c]?.value
				)
			  );
			...
			class FileViewerButton {
			  ...
			  render() {
				...
				const tooltip = new Tooltip(icon, {
				  text: `${
					this.open ? "Hide File Preview" : "Show"
				  }  File Preview`,
				  spacing: 8,
				});
				...
				if (!this.fileViewRef && this.open) {
				  ...
				  this.target
					.closest(`.${Classes.messageAttachment}`)
			...
			return class FileViwer extends Plugin {
			  ...
			  addViewerButton(attachmentElement, hoverButtonGroup) {
				const { url, filename, size } = getReactProps(attachmentElement);
				if (!url) {
				  return;
				}
  
				let isGoogleExtension = googleExtensions.some((e) => {
				  return filename.toLowerCase().endsWith(e);
				});
				...
				if (fileTooBig) {
				  ...
				  hoverButtonGroup.append(warningIcon);
			  ...
			  observer({ addedNodes }) {
				for (const added of addedNodes) {
				  ...
				  const hoverButtonGroup = added.getElementsByClassName(
					Classes.hoverButtonGroup
				  );
  
				  if (attachmentElement.length > 0) {
					Array.from(attachmentElement).forEach((attachment, index) => {
					  const buttonGroup = hoverButtonGroup[index];
					  if (
						attachment.getElementsByClassName("FileViewerButton")
						  .length > 0
					  )
						return;
					  this.addViewerButton(attachment, buttonGroup);
					});
				  }
				}
			  }

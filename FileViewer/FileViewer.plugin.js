...
module.exports = (() => {
		...
		changelog: [
			{
				...
				type: "added",
				items: [
					"Added the Office Web Apps Viewer as the default Viewer for office related files. Can be turned off in the settings.",
				]
			},
		],
	}
	return !global.ZeresPluginLibrary ? class {
		...
		const plugin = (Plugin, Library) => {
			...
			const WarningIcon = React.createElement("path", { fill: "red", d: "M12 0c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm-1.351 6.493c-.08-.801.55-1.493 1.351-1.493s1.431.692 1.351 1.493l-.801 8.01c-.029.282-.266.497-.55.497s-.521-.215-.55-.498l-.801-8.009zm1.351 12.757c-.69 0-1.25-.56-1.25-1.25s.56-1.25 1.25-1.25 1.25.56 1.25 1.25-.56 1.25-1.25 1.25z" });
					...
					let button = React.createElement(TooltipContainer, { text: this.state.displayingFile ? "Hide File" : "Show File" },
						React.createElement("svg", {
							class: "downloadButton-23tKQp", width: "24", height: "24", viewBox: "0 0 226 226", onClick: (e) => {
								if (e.shiftKey) { window.open(this.props.url, '_blank').focus(); return; }
			...
			return class FileViwer extends Plugin {
				start() {
					this.settings = this.loadSettings({ forceProvider: false })
				...
				patchAttachments() {
					Patcher.after("FileViewer", Attachment, "default", (_, __, ret) => {
						...
						let isGoogleExtension = googleExtensions.some((e) => { return fileUrl.endsWith(e) })
						let isOfficeExtension = officeExtensions.some((e) => { return fileUrl.endsWith(e) })
						if(!isGoogleExtension && !isOfficeExtension) { return;}
						...
						let button = React.createElement(FileViewerButton, { displayingFile: false, url: useGoogleProvider ? googleUrl : officeUrl });
						...
						if (fileTooBig) {
							warningIcon = React.createElement(TooltipContainer, { text: `This file is over 10mb! FileViewer might not be able to preview this file.` },
								React.createElement("svg", { width: "24", height: "24", viewBox: "0 0 24 24", style: { paddingLeft: "10px" } }, WarningIcon)
							);
						}
						ret.props.children = [
							...ret.props.children,
				...
				getSettingsPanel() {
					...
					return SettingPanel.build(() => this.saveSettings(this.settings),
						new Switch('Force Google provider', `By default all office related files will be displayed by the Office Web Apps Viewer, 
